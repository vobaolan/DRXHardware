    ...
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#090d16] text-slate-800 dark:text-slate-100 flex flex-col font-sans overflow-x-clip">
      ...
      <header className="bg-white dark:bg-slate-900 border-b border-slate-200/80 dark:border-slate-800 px-4 sm:px-6 py-3.5 flex flex-wrap items-center justify-between gap-4 sticky top-0 z-40 shadow-xs">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="font-heading text-sm sm:text-base font-black uppercase text-slate-900 dark:text-white tracking-wide truncate">
                CỔNG VẬN HÀNH STAFF & KHO LINH KIỆN
              </h1>
              <span className="px-2 py-0.5 rounded-full text-[9px] font-black uppercase bg-sky-50 dark:bg-sky-950/60 text-[#0284c7] dark:text-sky-400 border border-sky-200 dark:border-sky-800 flex items-center gap-1">
                <span className="w-1.5 h-1.5 rounded-full bg-[#0284c7] animate-pulse" />
                Staff Portal
              </span>
            </div>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate">
              Quản lý xuất nhập kho, tiến độ ráp PC và tra cứu bảo hành điện tử chính hãng.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            onClick={() => fetchAllStaffData(true)}
            ...
            title="Đồng bộ lại dữ liệu tức thì từ Database"
          >
          ...
          <Link
            href="/admin"
            className="inline-flex items-center gap-1 px-3 py-2 rounded-xl bg-slate-900 dark:bg-slate-800 hover:bg-[#0284c7] text-white font-bold text-xs transition-all shadow-xs"
            title="Chuyển sang trang Admin CEO Portal"
          >
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="text-[11px] text-slate-500 dark:text-slate-400 space-y-0.5">
                          ...
                          <p className="truncate" title={ord.shippingAddress}>Địa chỉ: {ord.shippingAddress || 'Nhận tại shop'}</p>
                          {ord.notes && <p className="italic text-slate-400">Ghi chú: "{ord.notes}"</p>}
                        </div>
                      </div>
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-1.5">
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 text-slate-700 dark:text-slate-300 text-[11px] font-bold cursor-pointer transition-all"
                        >
                          Chi Tiết
                        </button>
                        {ord.status !== 'COMPLETED' && (
                          <button
                            onClick={() => {
                              const nextStatus = ord.status === 'PENDING' ? 'CONFIRMED' : ord.status === 'CONFIRMED' ? 'SHIPPING' : 'COMPLETED';
                              handleUpdateOrderStatus(ord.id, nextStatus);
                            }}
                            className="px-3 py-1.5 rounded-lg bg-[#0284c7] hover:bg-[#0369a1] text-white text-[11px] font-bold cursor-pointer transition-all shadow-xs"
                          >
                            {ord.status === 'PENDING' ? 'Duyệt Ráp' : ord.status === 'CONFIRMED' ? 'Bàn Giao Vận Chuyển' : 'Hoàn Tất Nghiệm Thu'}
                          </button>
                        )}
                      </div>
                    </div>
