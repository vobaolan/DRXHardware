    ...
    >
      <Link href={`/products/${product.slug}`} className="flex flex-col h-full">
        ...
        <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-950">
          ...
          {hasDiscount && (
            <div className="absolute top-2.5 right-2.5 bg-gradient-to-r from-rose-500 to-amber-500 px-2.5 py-0.5 text-[10px] font-black text-white tracking-wider rounded-full shadow-lg z-20 border border-white/20">
              -{discountPercent}%
            </div>
          )}

          {product.brand && (
            <div className="absolute bottom-2.5 left-2.5 flex items-center gap-1.5 bg-slate-950/90 backdrop-blur-md px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider text-cyan-300 border border-cyan-500/30 shadow-lg z-20">
              <Cpu className="h-3 w-3 text-cyan-400" />
              <span>{product.brand}</span>
            </div>
          )}

          <div className="absolute bottom-2.5 right-2.5 bg-slate-950/90 backdrop-blur-md px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-wider text-emerald-300 border border-emerald-500/30 shadow-lg z-20 flex items-center gap-1.5">
