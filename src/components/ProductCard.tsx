...
export const ProductCard: React.FC<{ product: ProductProps }> = ({ product }) => {
  ...
  return (
    <motion.div
      ...
      className="uiverse-card-cyber flex flex-col group relative h-full cursor-pointer rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xs hover:shadow-xl hover:border-[#0284c7]/40 dark:hover:border-[#0284c7]/50 transition-all duration-300 overflow-hidden"
    >
      <Link href={`/products/${product.slug}`} className="flex flex-col h-full">
        {/* Product Media Box - Clean & Unobstructed */}
        <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-50 dark:bg-slate-950/80 p-3 flex items-center justify-center border-b border-slate-100 dark:border-slate-800/80">
          <img
          ...
          <div className="flex items-end justify-between pt-3 border-t border-slate-100 dark:border-slate-800/80 mt-auto gap-2">
            ...
            {product.status !== false ? (
              <button
                ...
                className="uiverse-btn-shimmer px-3.5 py-2 text-xs font-heading font-extrabold rounded-xl shadow-md z-30 relative cursor-pointer flex items-center gap-1.5 shrink-0"
              >
