...
export function OrderVerificationModal({ order, onClose, onOrderUpdated }: OrderVerificationModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/75 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl p-5 sm:p-7 max-w-3xl w-full space-y-6 shadow-2xl text-slate-900 dark:text-slate-100 my-8 relative overflow-hidden">
        
        {/* TOP AMBIENT ACCENT */}
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-[#0284c7] via-cyan-400 to-blue-600" />
          ...
          <div className="flex items-center gap-2 self-start sm:self-center">
            {currentStatus !== 'CANCELLED' && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-rose-300 dark:border-rose-900/60 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 text-rose-700 dark:text-rose-400 text-xs font-bold transition-all cursor-pointer shadow-xs"
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 hover:text-slate-800 dark:hover:text-slate-200 cursor-pointer transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
